...
# Last Update: 20141212

Troubleshooting
============
Q: synbak can't create html reports in servers with selinux enabled
A: before running first backup, we must enable right slinux context against web root:
...
A: synbak use the standard GNU gettext localization system
   ...
   4) cp synbak.pot to language.po (example: use 'de.po' for Deutsche language: cp synbak.pot de.po)
...
Q: I want make daily, weekly and monthly backups, what is the easiest and fastest method?
A1: You can use the backup_schedule function from config file, it's very simple:
   backup_schedule                 = yes
...
A2: (DEPRECATED AND TOO COMPLEX) You can use the -o command option to override every file configuration variable, so you can use only one config file for each system and change only the destination of backup.
   for example, into user crontab you can use the following commands:
...
A: The backup_exclude directive need directory paths or regexp patterns (this is valid for backup_source also).
   for examples:
...
Q: I'm attempting to backup local files to a remote smb filesystem, the FAQ shows how to do the opposite.
   Also, can I backup from a smb device to another smb device? 
A: synbak can act as backup server, collecting data 'from remote' host shares 'to local' synbak hard disks,
   but not 'from remote' host share 'to remote' host share, because you can specify only 'remote source' and
...
Q: I want make a backup of remote machine files to a local directory, what method can i use?
...
Q: I just use synbak rsync method to make my server's data secure, but, some servers are running live databases like MySQL, Oracle or LDAP directories
   how can i make a safe and clean database dump using synbak?
A: comment out 'backup_source_uri' and 'report_remote_uri_down' and use a URI like these:
   ...
   if the remote daemons are not running in the default port, you can specify the port like other URIs:
   backup_source_uri = mysql://username:password@hostname:3307

   by default, synbak make a single bzip2 compressed file dump with all databases, but for the MySQL method exist a special switch that allow you to make
   a splitted database dump for each database found (useful for easy single database recovery)
...
A: Firts you must discover your tape loader scsi device address name (generally /dev/sgX, where X is a number)
   ...
   and run a tape backup suppling the extra options:

   synbak -s myserver -m tape -M 1

   where 1 is the first tape number loaded in the tape changer

   nb. if your linux system use the udev daemon, discovering the right device node is pretty simple,
       normally is named /dev/changer
